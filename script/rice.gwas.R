...
rm(list = ls())
setwd("/data/users/gylee/RDA/lecture") #Change
Geno <- read_ped(paste0("input/","sativas413.ped"))
p = Geno$p
n = Geno$n
Geno = Geno$x
# Acession information
FAM <- read.table(paste0("input/","sativas413.fam"))
# Map information
MAP <- read.table(paste0("input/","sativas413.map"))
...
rice.pheno <- read.table(paste0("input/","RiceDiversity_44K_Phenotypes_34traits_PLINK.txt"), header = TRUE, stringsAsFactors = FALSE, sep = "\t")
...
snpgdsCreateGeno(paste0("output/","44k.gds"), genmat = Geno1, sample.id = sample, snp.id = snp.id,
    snp.chromosome = MAP1$V1, snp.position = MAP1$V4, snpfirstdim = FALSE)
# Now open the 44k.gds file
geno_44k <- snpgdsOpen(paste0("output/","44k.gds"))
snpgdsSummary(paste0("output/","44k.gds"))
...
# Plot the PCA
png(paste0("output/","pca3-4.png"))
p1 <- plot(pca$EV2, pca$EV1, xlab = "eigenvector 3", ylab = "eigenvector 4")
dev.off()
...
pca_1 <- read.csv(paste0("input/","RiceDiversity.44K.germplasm.csv"),
...
png(paste0("output/","pca1_2-legend.png"))
p2 <- plot(pca_population$EV1, pca_population$EV2, xlab = "PC1", ylab = "PC2", col = c(1:6)[factor(pca_population$population)])
...
corr.matrix12 <- cor(Geno1[, 31753:33701])
...
print(paste0("Threshold of Bonferroni correction : ",p_threshold))
...
png(paste0("output/","Manhattan_plot.png"))
p3 <- manhattan(x = GWAS_1, chr = "chrom", bp = "pos", p = "y", snp = "marker", col = c("blue4", 
